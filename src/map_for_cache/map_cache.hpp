...
public:
	...
	public:
		...
		iterator operator++(int) {
		    if(_pointer==_master_map->_end) throw invalid_iterator();
		    iterator temp(*this);
		...
		iterator operator--(int) {
            ...
            if(_pointer==nullptr) throw invalid_iterator();
            return temp;
		}
		iterator & operator--() {
            _pointer=_master_map->_get_precursor(_pointer);
	...
	T & at(const Key &key) {
	    ...
	    if(_temp==nullptr) throw index_out_of_bound();
	...
	std::pair<iterator, bool> insert(const value_type &value) {
	    ...
	    if(is_insert) _size++;
	    return pair<iterator,bool>(_res,is_insert);
	}
	void erase(iterator pos) {
	    if(pos._master_map!=this) throw invalid_iterator();
